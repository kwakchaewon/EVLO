...
### 현재 진행 상황
- **Week 2**: PDF/CSV/JSON 내보내기 기능 구현 완료 ✅
- **다음 작업**: 전역 예외 처리 및 에러 페이지
...
### 3.5 PDF 생성 기능
- [x] PDF 라이브러리 선정 및 통합
  - [x] OpenPDF 의존성 추가 (이미 추가됨)
  - [x] 라이브러리 테스트
- [x] 분석 보고서 PDF 템플릿 설계 (기본 테이블 형태)
  - [x] 헤더/푸터 간단 정보 (제목, 총 이벤트 수)
  - [x] 섹션 구성: 이벤트 리스트 테이블 (EventID, Level, Time, Provider, Computer, Message)
  - [ ] 차트 포함 (추후 Chart.js 이미지 변환 고려)
- [x] PDF 다운로드 API 구현
